...
TRAIN = Path("artifacts/dev3-teacher-gold-small-strict/train.jsonl")
VAL = Path("artifacts/dev3-teacher-gold-small-strict/eval.jsonl")
MODEL = "gpt-4.1-nano"
SUFFIX = "dev3-teacher-gold-v3"
EPOCHS = 2
URL = "https://tunesefoundry.openai.azure.com/openai/fine_tuning/jobs?api-version=2025-04-01-preview"
OUT = Path("artifacts/strict_teacher_gold_job.json")

print(f"Uploading strict train file: {TRAIN}", flush=True)
...
Path("artifacts/strict_teacher_gold_request.json").write_text(json.dumps(body, indent=2), encoding="utf-8")

cred = AzureCliCredential()
token = cred.get_token("https://cognitiveservices.azure.com/.default").token
print("Submitting strict fine-tune job...", flush=True)
